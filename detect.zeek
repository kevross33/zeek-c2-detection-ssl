...
# evaluate_encrypted_tunnel — SSL-independent long-tunnel detector.
#
# Catches a persistent encrypted C2 tunnel that Zeek joined mid-stream and
# therefore has NO ssl.log record (handshake missed — the connection predates
# the capture window or a Zeek/worker restart). Every SSL-based detector skips
# such a flow because it requires c$ssl; on a 24/7 sensor this is the common
# shape for an hours-long tunnel (e.g. Havoc over Microsoft Dev Tunnels).
#
# Works from connection metrics only. To stay FP-safe without SNI/cert/JA3 to
# reason about, it demands: a long duration, an encrypted-looking port, NO ssl
# record, a RARE destination (few clients), and a low-rate keep-alive or
# reverse-asymmetry shape. Allowlists still apply by IP/subnet.
function evaluate_encrypted_tunnel(c: connection)
    {
    if ( ! enc_tunnel_enabled )                       return;
    if ( triage_skip(c) )                             return;
    # This path is specifically for flows with NO SSL record. If the SSL
    # analyzer saw a handshake, the normal (richer) SSL detectors handle it.
    if ( c?$ssl )                                     return;
    if ( ! c?$duration )                              return;
    if ( c$id$resp_p !in enc_tunnel_ports )           return;

    local dur = interval_to_double(c$duration);
    if ( dur < interval_to_double(enc_tunnel_min_duration) )
        return;

    local orig_b = c?$orig && c$orig?$size ? c$orig$size : 0;
    local resp_b = c?$resp && c$resp?$size ? c$resp$size : 0;
    local total_b = orig_b + resp_b;
    if ( total_b < enc_tunnel_min_bytes )
        return;   # too little exchange to distinguish from benign idle keep-alive

    # Must be bidirectional — a real tunnel carries data both ways.
    if ( orig_b == 0 || resp_b == 0 )
        return;

    # Destination rarity: benign long-lived :443 (streaming, SaaS, websockets)
    # is overwhelmingly to POPULAR destinations (many clients). A persistent
    # single-client encrypted tunnel to a rare dest is the C2 shape. We key
    # rarity on the destination IP (no SNI is available here).
    local dest_key = fmt("ip:%s", c$id$resp_h);
    local n_clients = (dest_key in dest_client_count) ?
                      |dest_client_count[dest_key]| : 0;
    if ( n_clients > enc_tunnel_max_dest_clients )
        return;

    local bps = dur > 0.0 ? (total_b + 0.0) / dur : 0.0;

    local indicators: set[string] = set();
    add indicators["encrypted_tunnel_no_handshake"];
    add indicators["mid_stream_join"];
    if ( n_clients <= 1 )
        add indicators["single_client_destination"];

    local conf = enc_tunnel_base_confidence;

    # Keep-alive shape: sustained, low-rate — a control channel, not bulk.
    if ( bps <= enc_tunnel_max_bps )
        {
        conf += 0.15;
        add indicators["low_rate_keepalive"];
        }

    # Interactive-shell shapes (same as the SSL tunnel path).
    ...
    # Host-correlation: if this originator is already confirmed compromised,
    # a further persistent encrypted tunnel is strong corroboration.
    if ( conf >= host_escalation_floor && is_host_compromised(c$id$orig_h) )
        {
        conf += host_compromise_bonus;
        add indicators["compromised_host_activity"];
        }

    # Threat-intel corroboration on the destination IP.
    if ( intel_corroboration_enabled )
        {
        local idesc = intel_hit_for_dest(c$id$resp_h);
        if ( idesc != "" )
            {
            conf += intel_corroboration_bonus;
            add indicators[fmt("intel_hit:%s", idesc)];
            }
        }
    ...
    local det_cat = SSL_TUNNEL_KEEPALIVE;
    if ( "reverse_asymmetry_shell" in indicators ||
         "long_lived_idle_shell" in indicators )
        det_cat = SSL_REVERSE_FLOW_RAT;

    local details = fmt("enc-tunnel no-handshake dur=%.0fs orig=%d resp=%d bps=%.0f port=%s clients=%d",
                        dur, orig_b, resp_b, bps, c$id$resp_p, n_clients);

    emit_from_conn(det_cat, conf, c, "(no-sni)", F, details, indicators,
                   INNER_UNKNOWN);

    if ( conf >= host_compromise_entry_confidence )
        mark_host_compromised(c$id$orig_h, "", "");
    }
...
event connection_state_remove(c: connection) &priority = -5
    ...
    # SSL-independent path: a long-lived encrypted flow with NO ssl.log
    # record (handshake missed / mid-stream join). Track destination client
    # population by IP so the rarity test has data, then evaluate. This runs
    # BEFORE the c$ssl gate below, which would otherwise drop these flows.
    if ( enc_tunnel_enabled && ! c?$ssl && c?$duration &&
         c$id$resp_p in enc_tunnel_ports &&
         interval_to_double(c$duration) >= interval_to_double(enc_tunnel_min_duration) )
        {
        note_dest_client(fmt("ip:%s", c$id$resp_h), c$id$orig_h);
        evaluate_encrypted_tunnel(c);
        }
